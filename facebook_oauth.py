...
class FacebookOAuthHandler:
    ...
    def initiate_facebook_login(self, request: Request) -> RedirectResponse:
        ...
        authorize_url = self.oauth_config.facebook_oauth_client.create_authorization_url(
            url=self.oauth_config.facebook_auth_url,
            state=state,
            scope="email public_profile", # Request necessary scopes
            redirect_uri=self.oauth_config.facebook_redirect_uri
        )
...
class FacebookOAuthHandler:
    ...
    async def handle_facebook_callback(
        ...
        try:
            token_response = await self.oauth_config.facebook_oauth_client.fetch_token(
                url=self.oauth_config.facebook_token_url,
                code=code,
                redirect_uri=self.oauth_config.facebook_redirect_uri,
            ...
            user_info = await self.fetch_facebook_user_profile(access_token)
    ...
    async def fetch_facebook_user_profile(self, access_token: str) -> Dict[str, Any]:
        ...
        try:
            # Facebook Graph API requires specific fields
            userinfo_response = requests.get(
                f"{self.oauth_config.facebook_userinfo_url}?fields=id,name,email,picture",
